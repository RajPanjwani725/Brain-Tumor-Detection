...
model_dir = tf.keras.models.load_model('models_video')
detection_status = ""
detection_message = ""
...
def load_inference():
    ...
    detection_datetime = str(detection_datetime)
    ...
    for i in y_sample_prob:
        for j in i:
            ...
            if j > 60.00:
                ...
                detection_probabiliy = "    Tumor Detected   "
                ...
                while j < 1:
                    ...
                    j = j * 10
                j = "{:.2f}".format(j)
                detection_probabiliy = "  No Tumor Detected  "
                detection_probabiliy_percentage = str(j) + "%"
    ...
    cursor.execute(
        "UPDATE dataset_table SET detection_outputfileName='" + output_filename + "', detection_status ='"+detection_message+"' ,detection_datetime='"+detection_datetime+"' where id =" + id)
    print("Updated")
...
def output_display(new_image, color, output_file):  #
    ...
    new_image = cv2.resize(new_image, (500, 350), interpolation=cv2.IMWRITE_JPEG_CHROMA_QUALITY)
